...
import { isContextCompacting } from "@t3tools/client-runtime/state/compaction";
  ...
  const isCompacting = useMemo(() => {
    ...
    return isContextCompacting({
      ...
      pendingMessage: queuedMessage
        ? { ...queuedMessage, id: queuedMessage.messageId, role: "user" }
        : undefined,
    });
  }, [
  ...
  const onSendMessage = useCallback(
    async (deliveryMode?: TurnDeliveryMode) => {
      ...
      const enqueuePromise = enqueueThreadOutboxMessage({
        ...
        ...(provider?.driver === "pi" && deliveryMode !== undefined ? { deliveryMode } : {}),
        createdAt: metadata.createdAt,
      });
