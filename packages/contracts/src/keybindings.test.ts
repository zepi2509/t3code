...
it.effect("parses keybinding rules", () =>
  Effect.gen(function* () {
    ...
    const parsedQueuedSend = yield* decode(KeybindingRule, {
      key: "mod+enter",
      command: "composer.sendAfterCompletion",
    });
    assert.strictEqual(parsedQueuedSend.command, "composer.sendAfterCompletion");
